...
This setting is important for real-time surveillance, where delayed detection can reduce practical utility. Accordingly, detection delay is an important complementary evaluation criterion for online VAD. Beyond whether an anomalous event is eventually detected, an online system should also be evaluated according to how quickly it raises an alarm after anomaly onset. One such metric is Average Detection Delay (ADD) [46], defined over $N$ anomalous activities as

$$ADD = \frac{1}{N}\sum_{i=1}^{N} (T_i - \tau_i) \qquad (27)$$

where $\tau_i$ denotes the onset time of anomalous activity $i$ and $T_i \geq \tau_i$ denotes the corresponding alarm time. If no alarm is produced within a predefined maximum tolerable delay $\delta_{\max}$, the delay for that event can be capped at $\delta_{\max}$. Two methods with similar frame-level AUC or AP may therefore differ substantially in practical utility if one consistently detects anomalous events earlier than the other. Doshi and Yilmaz further combine normalized ADD with alarm precision through the Average Precision Delay (APD) metric, jointly accounting for timely detection and false alarms [46]. Although such latency-aware metrics have been proposed, detection-delay evaluation is not yet standardized across VAD benchmarks.
