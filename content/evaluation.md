...
$$TPR = \frac{TP}{TP+FN}, \qquad FPR = \frac{FP}{FP+TN} \qquad (28)$$
...
$$AUC = \int_0^1 TPR(FPR)\, d(FPR) \qquad (29)$$
...
$$\hat{y}_t = \begin{cases} 1, & s_t > \tau, \\ 0, & \text{otherwise.} \end{cases} \qquad (32)$$
...
$$IoU(R_{gt}, R_{det}) = \frac{|R_{gt} \cap R_{det}|}{|R_{gt} \cup R_{det}|} \qquad (36)$$

A ground-truth region is counted as detected if

$$IoU(R_{gt}, R_{det}) \geq \beta \qquad (37)$$

The corresponding region-based detection rate (RBDR) is then computed as

$$RBDR = \frac{\text{number of anomalous regions detected}}{\text{total number of anomalous regions}} \qquad (38)$$
...
$$TBDR = \frac{|\mathcal{T}_{det}|}{|\mathcal{T}_{gt}|} \qquad (39)$$
...
$$FPR = \frac{\text{total false positive regions}}{\text{total frames}} \qquad (40)$$
...
$$BLEU = BP \cdot \exp\left(\sum_{n=1}^{N} w_n \log p_n\right) \qquad (41)$$
...
$$ROUGE\text{-}N = \frac{\sum \text{N-gram}\ Count_{match}}{\sum \text{N-gram}\ Count_{reference}} \qquad (42)$$
...
$$METEOR = \frac{10PR}{R+9P} \qquad (43)$$
...
$$Score_{LLM} = f_{LLM}(\hat{y}, r, p) \qquad (44)$$
...
$$Score_{MLLM} = f_{MLLM}(V, \hat{y}, r, p) \qquad (45)$$
